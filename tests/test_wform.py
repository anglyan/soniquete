...
from soniquete.wform import Tone, Waveform
...
def test_sample_count_matches_duration_and_sample_rate():
    wf = Waveform(frequencies=[Tone(440, 1.0)], duration=0.5, sample_rate=1000)
...
def test_intensity_scales_peak_amplitude():
    ...
    wf = Waveform(frequencies=[Tone(25, 1.0)], intensity=0.5, duration=1.0, sample_rate=100)
    assert np.max(np.abs(wf._wform)) == pytest.approx(0.5)


def test_multiple_tones_are_amplitude_weighted_and_normalized():
    wf = Waveform(
        frequencies=[Tone(10, 1.0), Tone(10, 3.0)],
        intensity=1.0,
        duration=1.0,
        sample_rate=100,
    )
...
def test_accepts_frequency_instances_and_note_names():
    wf = Waveform(frequencies=[Tone(Frequency("A4"), 1.0)], duration=1.0, sample_rate=100)
    assert wf.frequencies[0].hz == pytest.approx(440.0)


def test_phase_shifts_the_tone():
    wf = Waveform(
        frequencies=[Tone(10, 1.0, phase=np.pi / 2)],
        intensity=1.0,
        duration=1.0,
        sample_rate=100,
    )
    t = np.arange(100) / 100
    expected = np.sin(2 * np.pi * 10 * t + np.pi / 2)
    assert wf._wform == pytest.approx(expected)
...
def test_array_property_returns_wform():
    wf = Waveform(frequencies=[Tone(440, 1.0)], duration=0.1)
...
def test_non_tone_item_raises():
    with pytest.raises(TypeError):
        Waveform(frequencies=[(440, 1.0)])
